 ...
 * Format attributes
 */
function formatAttributes(attributes: Record<string, string | null> | undefined, opts: DomSerializerOptions) {
  ...
  return Object.keys(attributes)
    .map(key => {
      ...
      return `${key}="${opts.decodeEntities !== false ? encodeXML(value) : value.replace(/"/g, '&quot;')}"`;
    })
    .join(' ');
}
 ...
 */
export default function serializer(node: RaisinNode | RaisinNode[], options: DomSerializerOptions = {}): string {
  const nodes: RaisinNode[] = Array.isArray(node) ? (node as RaisinNode[]) : [node];
...
function renderNode(node: RaisinNode, options: DomSerializerOptions, parents: Parents): string {
  return visit(node, {
...
function renderTag(elem: RaisinElementNode, opts: DomSerializerOptions, parents: Parents, children: string[]) {
  const parent = parents.get(elem);
  let { tagName } = elem;
  // Handle SVG / MathML in HTML
  if (opts.xmlMode === 'foreign') {
    /* Fix up mixed-case element names */
    tagName = elementNames.get(elem.tagName) ?? elem.tagName;
    /* Exit foreign mode at integration points */
    if (parent && foreignModeIntegrationPoints.has((parent as RaisinElementNode).tagName)) {
...
function renderText(elem: RaisinTextNode, opts: DomSerializerOptions, parents: Parents) {
  let data = elem.data || '';
  const parent = parents.get(elem);
  // If entities weren't decoded, no need to encode them back
  if (opts.decodeEntities !== false && !(!opts.xmlMode && parent && unencodedElements.has((parent as RaisinElementNode).tagName))) {
    data = encodeXML(data);
  }
...
function renderCdata(elem: RaisinNodeWithChildren) {
  return `<![CDATA[${(elem.children[0] as RaisinProcessingInstructionNode).data}]]>`;
}
